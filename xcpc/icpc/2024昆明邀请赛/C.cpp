#include <bits/stdc++.h>
using namespace std;
using i64 = long long;
using u32 = unsigned;
using i128 = __int128;  

template<class T>
struct MaxFlow {
    struct Edge {
        int y;
        //剩余流量
        T cap;
        Edge(int y_, T cap_) : y(y_), cap(cap_) {};
    };

    int n;
    vector<Edge> e;
    vector<vector<int>> g;
    vector<int> cur, d;
    MaxFlow(int n_) : n(n_), g(n + 1) {};

    int addEdge(int x, int y, int cap) {
        g[x].emplace_back(e.size());
        e.emplace_back(y, cap);
        g[y].emplace_back(e.size());
        e.emplace_back(x, 0);
        return e.size() - 2;
    }

    // 在原图基础上bfs得到层次图
    bool bfs(int stt, int edd) {
        d.assign(n + 1, -1);
        d[stt] = 0;
        queue<int> q;
        q.push(stt);
        while(q.size()) {
            int x = q.front();
            q.pop();

            for(auto j : g[x]) {
                auto [y, cap] = e[j];
                if(cap == 0 || d[y] != -1) continue;
                d[y] = d[x] + 1;
                if(y == edd) return true;
                q.push(y);
            }
        }
        return false;
    }

    // 在层次图中dfs出阻塞流
    // limit最大入流，flow还可以流出的流量，limit - flow真实出流
    T dfs(int x, int edd, T limit) {
        if(x == edd) return limit;
        T flow = limit;
        for(int &i = cur[x]; i < g[x].size(); i ++) {
            int j = g[x][i];
            auto [y, cap] = e[j];

            if(d[y] != d[x] + 1 || cap == 0) continue;
            // 下一个点的最大入流等于min(当前点还可以流出的流量，当前边的剩余容量)
            T t = dfs(y, edd, min(flow, cap));
            e[j].cap -= t;
            e[j ^ 1].cap += t;
            flow -= t;
            if(flow == 0) return limit;
        }
        return limit - flow;
    }

    T flow(int stt, int edd) {
        T flow = 0; 
        while(bfs(stt, edd)) {
            cur.assign(n + 1, 0);
            flow += dfs(stt, edd, numeric_limits<T>::max());
        }
        return flow;
    }

    struct _Edge {
        int x;
        int y;
        //最大容量
        T cap;
        T flow;
    };

    vector<_Edge> edges() {           //仅限单向边
        vector<_Edge> res;
        for(int i = 0; i < e.size(); i += 2) {
            res.emplace_back(e[i + 1].y, e[i].y, e[i].cap + e[i + 1].cap, e[i + 1].cap);
        }
        return res;
    }

    void reset() {
        for(int i = 0; i < e.size(); i += 2) {
            e[i].cap += e[i + 1].cap;
            e[i + 1].cap = 0;
        }
    }
};

struct Node {
    int x, id;
    bool iscb;
};

struct Edge {
    int x, y;
    int zid;
    int eid;
};

void solve() {
    int n, m, k;
    cin >> n >> m >> k;
    int keep = m - k;

    map<int, vector<Node>> mpx;
    map<int, vector<Node>> mpy;

    for(int i = 1; i <= n; i ++) {
        int x, y;
        cin >> x >> y;
        mpx[x].push_back({y, -1, true});
        mpy[y].push_back({x, -1, true});
    }

    for(int i = 1; i <= m; i ++) {
        int x, y;
        cin >> x >> y;
        mpx[x].push_back({y, i, false});
        mpy[y].push_back({x, i, false});
    }

    auto work = [&](auto &mp, int &cnt) {
        vector<int> bel(m + 1, -1);
        for(auto [_, node] : mp) {
            sort(node.begin(), node.end(), 
                [&](auto &t, auto &ano) {
                    return t.x < ano.x;
                });
            int lst = -1;
            for(int i = 0; i < node.size(); i ++) {
                if(node[i].iscb) {
                    if(lst != -1) {
                        cnt ++;
                        for(int j = lst + 1; j < i; j ++) {
                            bel[node[j].id] = cnt;
                        }
                    }
                    lst = i;
                }
            }
        }
        return bel;
    };

    int cntx = 0, cnty = 0;
    auto belx = work(mpx, cntx);
    auto bely = work(mpy, cnty);

    int S = 0, T = cntx + cnty + 1;
    MaxFlow<int> mf(cntx + cnty + 1);

    for(int i = 1; i <= cntx; i ++) {
        mf.addEdge(S, i, 1);
    }
    for(int i = 1; i <= cnty; i ++) {
        mf.addEdge(cntx + i, T, 1);
    }

    vector<Edge> add;
    for(int i = 1; i <= m; i ++) {
        int x = belx[i], y = bely[i];
        if(x != -1 && y != -1) {
            int eid = mf.addEdge(x, cntx + y, 1);
            add.push_back({x, y, i, eid});
        }
    }

    int flow = mf.flow(S, T);
    vector<int> visx(cntx + 1), visy(cnty + 1);
    vector<int> visz(m + 1);

    for(auto &[x, y, zid, eid] : add) {
        if(mf.e[eid].cap == 0 && keep) {
            keep --;
            visx[x] = true;
            visy[y] = true;
            visz[zid] = true;
        }
    }

    if(keep) {
        for(int i = 1; i <= m; i ++) {
            if(visz[i]) {
                continue;
            }
            if(!keep) {
                break;
            }
            int x = belx[i], y = bely[i];
            if(x != -1 && !visx[x]) {
                keep --;
                visx[x] = true;
                visz[i] = true;
            } else if(y != -1 && !visy[y]) {
                keep --;
                visy[y] = true;
                visz[i] = true;
            }
        }
    }

    if(keep) {
        for(int i = 1; i <= m; i ++) {
            if(visz[i]) {
                continue;
            }
            if(!keep) {
                break;
            }
            visz[i] = true;
            keep --;
        }
    }

    int ans = cntx + cnty;
    for(int x = 1; x <= cntx; x ++) {
        if(visx[x]) {
            ans --;
        }
    }
    for(int y = 1; y <= cnty; y ++) {
        if(visy[y]) {
            ans --;
        }
    }

    cout << ans << '\n';

    for(int i = 1; i <= m; i ++) {
        if(!visz[i]) {
            cout << i << ' ';
        }
    }
    cout << '\n';
}

int main() {
    ios::sync_with_stdio(0);
    cin.tie(0);

    int t = 1;
    cin >> t;
    while(t --) {
        solve();
    }

    return 0;
}