...
int main()
{
    int n, m;
    cin >> n >> m;
    MaxFlow<int> mf(2 * n + 1);
    int stt = 0, edd = 2 * n + 1;
    for(int i = 1; i <= n; i ++)
    {
        int x;
        cin >> x;
        mf.addEdge(n + i, edd, x);
    }
    for(int i = 1; i <= n; i ++)
    {
        int x;
    ...
    {
        if(cap == flow && x == stt) cnt ++;
        if(cap == flow && y == edd) cnt ++;
    }    
    cout << cnt << '\n';
    for(auto &[x, y, cap, flow] : edges)
    {
        if(cap == flow && x == stt) cout << y << ' ' << '-' << '\n';
        if(cap == flow && y == edd) cout << x - n << ' ' << '+' << '\n';
    }
